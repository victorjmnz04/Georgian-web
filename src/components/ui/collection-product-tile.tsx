        ...
        <div className="mt-auto border-t border-black/7 px-4 pb-5 pt-4 md:px-5">
          <div className="flex items-start justify-between gap-5">
            <div className="space-y-2">
              ...
              <h3 className="font-serif text-[1.55rem] leading-[1.02] text-[var(--color-ink)]">
                {product.name}
              </h3>
          ...
          <div className="mt-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              {product.colors.map((color) => (
                <span
                  className={`h-2.5 w-2.5 rounded-full border border-black/12 transition-transform ${
                    color.name === activeColor.name ? "scale-110" : ""
                  }`}
                  key={color.name}
